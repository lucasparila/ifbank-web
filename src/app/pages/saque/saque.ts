...
export class Saque implements OnInit {
  ...
  sacar(): void {
    ...
    if (this.perfil.conta && this.valor > this.perfil.conta.saldo) {
      this.mensagem = 'Saldo insuficiente para este saque.';
      this.tipoMensagem = 'danger';
      return;
    }
    ...
    this.movimentacaoService.sacar({ idConta: this.contaId, valor: this.valor }).subscribe({
      next: (res) => {
        if (this.perfil.conta) {
          this.perfil.conta.saldo = res.saldoAtualizado;
        }
        this.mensagem = `Saque de R$ ${Number(res.valor).toFixed(2).replace('.', ',')} realizado com sucesso!`;
      ...
      error: (err) => {
        this.mensagem = typeof err.error === 'string' 
          ? err.error 
          : 'Erro ao realizar saque. Tente novamente.';
        this.tipoMensagem = 'danger';
        this.processando = false;
        this.cdr.detectChanges();
      },
